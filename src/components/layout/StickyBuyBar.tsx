...
export const StickyBuyBar: React.FC = () => {
  ...
  return (
    <aside 
      ...
      aria-hidden={!visible}
      className={`fixed bottom-0 left-0 right-0 z-50 backdrop-blur-md bg-night-950/85 text-bone border-t border-white/10 shadow-[0_-10px_30px_rgba(0,0,0,0.6)] md:hidden transition-transform ease-mech-s ${
        prefersReduced ? 'duration-0' : 'duration-300'
      } ${
        visible ? 'translate-y-0 pointer-events-auto' : 'translate-y-full pointer-events-none'
      }`}
    >
      <div className="max-w-md mx-auto px-3 h-14 flex items-center justify-between gap-3">
        ...
        {/* Action Button: Minimum 52px height as required */}
        <a
          href="#oferta"
          className="h-[52px] px-6 bg-accent hover:bg-accent-hover text-white font-semibold text-sm tracking-wide uppercase flex items-center justify-center transition-colors flex-shrink-0"
        >
          Lo quiero
        </a>
      </div>
