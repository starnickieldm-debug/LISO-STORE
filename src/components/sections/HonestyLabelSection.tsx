      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-8 lg:px-12 relative z-10">
          ...
          <div className="relative">
            ...
            <div className="border-y border-graphite/10 py-3.5 mb-8 lg:mb-10">
              <div className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-2">
                <h3 className="font-display text-2xl sm:text-3xl lg:text-4xl font-bold uppercase tracking-tight text-graphite">
            ...
            <div className="hidden md:grid grid-cols-12 gap-8 lg:gap-10 xl:gap-12 pb-10 border-b border-graphite/10 items-stretch">
              ...
              <div className="col-span-12 lg:col-span-4 flex flex-col items-center justify-center relative group py-2">
                ...
                <div className="w-full max-w-[320px] xl:max-w-[340px] grid grid-cols-3 gap-2 font-sans mt-2">
                  <div className="bg-white/80 backdrop-blur-sm border border-graphite/10 rounded-xl p-2.5 sm:p-3 text-center shadow-2xs hover:border-graphite/25 transition-colors">
                    <span className="block text-[11px] sm:text-xs uppercase tracking-wider text-graphite/60 font-medium">Potencia</span>
                ...
                <div className="space-y-4">
                  ...
                  <ul className="space-y-4 pt-1">
                    {honestyLabelData.does.map((item, idx) => (
                      <li 
                        key={idx} 
                        className="flex items-start gap-3 text-graphite leading-relaxed transition-all duration-200 ease-mech-s"
                      >
                        <Check className="w-4 h-4 text-emerald-700 flex-shrink-0 mt-1 stroke-[2.5]" />
                        <div className="space-y-1">
                          <p className="font-bold text-sm sm:text-base lg:text-[17px] text-graphite leading-snug">{item.title}</p>
                          <p className="text-graphite/75 text-xs sm:text-sm lg:text-[15px] leading-relaxed">{item.desc}</p>
                        </div>
                      </li>
                    ))}
                  </ul>
            ...
            <div className="block md:hidden pb-6 border-b border-graphite/10">
              ...
              <div className="relative w-full max-w-[320px] mx-auto mb-4 flex flex-col items-center">
                ...
                <div className="relative z-10 mb-2">
                  <span className="inline-flex items-center gap-1.5 px-3.5 py-1 bg-graphite/[0.04] border border-graphite/12 text-graphite/80 text-[11px] font-sans font-bold uppercase tracking-wider rounded-full shadow-xs">
                ...
                <p className="text-xs sm:text-[13px] font-sans text-graphite/60 mt-1.5 font-medium text-center">
                  Placa giratoria 90° · Pantalla LED en vivo · Depósito 100 ml
                </p>
              </div>

              {/* 2. Capsule Segmented Tab Switcher (Liquid+ Style) */}
              <div className="flex items-center p-1 bg-graphite/[0.05] border border-graphite/15 rounded-full mb-4 max-w-[360px] mx-auto shadow-inner">
                <button
                  type="button"
                  onClick={() => setMobileTab('does')}
                  className={`flex-1 py-2.5 px-3 text-center rounded-full font-sans text-xs sm:text-sm font-bold tracking-wide transition-all duration-200 cursor-pointer ${
                    mobileTab === 'does'
              ...
              <div className="bg-white border border-graphite/10 rounded-2xl p-5 sm:p-6 shadow-xs">
                <div className="pb-3 mb-3 border-b border-graphite/10 flex items-center justify-between">
                  <h4 className="font-display font-bold text-base sm:text-lg text-graphite tracking-tight">
                    {mobileTab === 'does' ? 'Donde LISO realmente brilla:' : 'Para lo que no está hecha:'}
                  </h4>
                  <span className={`text-xs font-sans font-bold uppercase px-2.5 py-0.5 rounded-full ${
                ...
                {/* Active Tab List */}
                {mobileTab === 'does' ? (
                  <ul className="space-y-4 pt-1 animate-fadeIn">
                    {honestyLabelData.does.map((item, idx) => (
                      <li key={idx} className="flex items-start gap-3 text-graphite leading-relaxed">
                        ...
                        <div className="space-y-0.5">
                          <p className="font-bold text-sm sm:text-base text-graphite leading-snug">{item.title}</p>
                          <p className="text-graphite/75 text-xs sm:text-[13.5px] leading-relaxed">{item.desc}</p>
                        </div>
                      </li>
                    ))}
                  </ul>
            ...
            {/* Label Footer Note */}
            <div className="mt-8 pt-4 border-t border-graphite/10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 text-xs sm:text-[13px] font-sans font-medium text-graphite/60">
        ...
        <Reveal direction="up" delay={150} duration={600}>
          <div className="mt-8 text-center max-w-xl mx-auto">
            <p className="text-sm sm:text-base text-graphite/70 italic">
