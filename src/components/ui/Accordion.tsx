  ...
  return (
    <div className={`border-b transition-colors ${isDarkTheme ? 'border-night-700' : 'border-graphite/15'}`}>
      <button
        type="button"
        onClick={onToggle}
        className="w-full py-5 sm:py-6 text-left flex items-start justify-between gap-4 focus:outline-none group cursor-pointer"
      ...
      >
        <div className="flex items-baseline space-x-3 sm:space-x-4">
          ...
          <span className={`text-base sm:text-lg font-medium transition-colors leading-snug ${
            isDarkTheme 
        ...
        <div className={`mt-1 flex-shrink-0 w-6 h-6 flex items-center justify-center transition-transform duration-200 ${
      ...
      >
        <div className="overflow-hidden">
          <div className={`pb-6 pl-7 sm:pl-10 pr-4 text-base leading-relaxed ${isDarkTheme ? 'text-bone/80' : 'text-graphite/80'}`}>
            <p>{item.answer}</p>
            {!item.verified && (
              <div className={`mt-3 inline-flex items-center space-x-2 px-2.5 py-1 text-[11px] font-mono font-medium ${
                isDarkTheme 
